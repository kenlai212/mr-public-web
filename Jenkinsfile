        ...
        stage('Deploy') {
            steps {
                sshagent(credentials : ['kenlai212']) {
                    ...
                    sh "ssh -l " + env.TARGET_DEPLOY_SERVER_SSH_ACCOUNT + " " + env.TARGET_DEPLOY_SERVER_IP + " 'docker pull kenlai212/mr-public-web:sandbox && docker stop mr-public-web || true && docker rm mr-public-web || true && docker run -d --name mr-public-web -p 3000:3000 --network app-network kenlai212/mr-public-web:sandbox'"
                }
            }
        }
    }
