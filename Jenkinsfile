        ...
        stage('Deploy') {
            ...
            steps {
                sshagent(credentials : ['kenlai212']) {
                    ...
                    sh "ssh -l " + env.TARGET_DEPLOY_SERVER_SSH_ACCOUNT + " " + env.TARGET_DEPLOY_SERVER_IP + " 'docker pull " + env.DOCKER_IMAGE + ":" + env.BUILD_TAG + " && docker stop " + env.CONTAINER_NAME + " || true && docker rm " + env.CONTAINER_NAME + " || true && docker run -d --name " + env.CONTAINER_NAME + " -p " + env.CONTAINER_PORT_MAPPING + " --network app-network "+ env.DOCKER_IMAGE +":" + env.BUILD_TAG + "'"
                }
            }
        }
    }
